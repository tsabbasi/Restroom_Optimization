...
%  wait time is essentially equal to departure time of person(s) using BR
...
for i=1:numel(arrival_times)
    ...
    if i <= bathroom_units
        % current departure time = arrival time + time spent in BR        
    ...
    elseif arrival_times(i) < val
        % curr. depart time = min depart time frm occupied BRs + time spent
